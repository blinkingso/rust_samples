...
use nacos_common::grpc_client::request::{
    ConnectionSetupRequest, HealthCheckRequest, Request, ServerCheckRequest,
};
use nacos_common::grpc_client::GrpcClient;
use nacos_common::grpc_utils;
...
extern crate nacos_common;
use nacos_common::grpc_service::*;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let response = client
        .request(tonic::Request::new(grpc_utils::convert_request(
    ...
    let payload = grpc_utils::convert_request(&setup_request);
    ...
    let health_check = HealthCheckRequest::new(req);
    let response = client
        .request(tonic::Request::new(grpc_utils::convert_request(
            &health_check,
        )))
